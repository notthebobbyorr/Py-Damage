...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--loc-model", type=Path, default=Path(__file__).resolve().parent / "models" / "is_swing_catboost_model_with_locations.cbm")
    parser.add_argument("--base-model", type=Path, default=Path(__file__).resolve().parent / "models" / "is_swing_catboost_model.cbm")
    parser.add_argument("--whiff-loc-model", type=Path, default=Path(__file__).resolve().parent / "models" / "p_whiff_v2_with_locations.cbm")
    parser.add_argument("--whiff-base-model", type=Path, default=Path(__file__).resolve().parent / "models" / "p_whiff_v2.cbm")
    parser.add_argument("--pitcher-output", type=Path, default=Path(__file__).resolve().parent / "data" / "raw" / "pitcher_p_swstr.parquet")
    parser.add_argument("--pitch-type-output", type=Path, default=Path(__file__).resolve().parent / "data" / "raw" / "pitch_types_p_swstr.parquet")
