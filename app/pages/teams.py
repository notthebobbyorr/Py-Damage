...
from app.datasets import (
    ...
    team_damage,
    team_hitter_splits_df,
    team_pitcher_splits_df,
    team_stuff,
)
...
def _team_hitting_splits():
    if team_hitter_splits_df.empty:
        st.info("Missing team_hitter_splits.csv — run the daily pipeline to generate it.")
        return

    tabs = st.tabs(list(_TEAM_SPLIT_TABS.keys()))
    for idx, tab_name in enumerate(_TEAM_SPLIT_TABS.keys()):
        with tabs[idx]:
            split_type = _TEAM_SPLIT_TABS[tab_name]
            split_df = team_hitter_splits_df[
                team_hitter_splits_df["split_type"] == split_type
            ].copy()
            if split_df.empty:
                st.info(f"No data for {tab_name}.")
                continue
            ...
            with left:
                ...
                season = st.multiselect(
                    ...
                    key=f"team_hitting_splits_season_{idx}",
                )
                game_type_group = st.selectbox(
                    "Game Type",
                    game_type_group_options(team_hitter_splits_df),
                    index=0,
                    key=f"team_hitting_splits_game_type_group_{idx}",
                )
                split_choice = st.multiselect(
                    "Select Split",
                    ["All"] + sorted(split_df["split"].dropna().unique().tolist()),
                    default=["All"],
                    key=f"team_hitting_splits_split_{idx}",
                )
                team = st.selectbox(
                    "Select Team",
                    team_options(split_df, "hitting_code"),
                    index=0,
                    key=f"team_hitting_splits_team_{idx}",
                )
            with right:
                if game_type_group != "Regular Season":
                    st.info(GAME_TYPE_GROUP_NOTE.format(game_type_group))
                ...
                render_table(
                    ...
                    stats_df=stats_df,
                    include_team_label=True,
                    label_cols=["Team", "Split"],
                )
                download_button(
                    df, f"team_hitting_splits_{split_type}", f"team_hitting_splits_download_{idx}"
                )


def _team_pitching_splits():
    if team_pitcher_splits_df.empty:
        st.info("Missing team_pitcher_splits.csv — run the daily pipeline to generate it.")
        return
    ...
    for idx, tab_name in enumerate(_TEAM_SPLIT_TABS.keys()):
        with tabs[idx]:
            split_type = _TEAM_SPLIT_TABS[tab_name]
            split_df = team_pitcher_splits_df[
                team_pitcher_splits_df["split_type"] == split_type
            ].copy()
            if split_df.empty:
                st.info(f"No data for {tab_name}.")
                continue
            ...
            with left:
                ...
                season = st.multiselect(
                    ...
                    key=f"team_pitching_splits_season_{idx}",
                )
                game_type_group = st.selectbox(
                    "Game Type",
                    game_type_group_options(team_pitcher_splits_df),
                    index=0,
                    key=f"team_pitching_splits_game_type_group_{idx}",
                )
                split_choice = st.multiselect(
                    "Select Split",
                    ["All"] + sorted(split_df["split"].dropna().unique().tolist()),
                    default=["All"],
                    key=f"team_pitching_splits_split_{idx}",
                )
                team = st.selectbox(
                    "Select Team",
                    team_options(split_df, "pitching_code"),
                    index=0,
                    key=f"team_pitching_splits_team_{idx}",
                )
            with right:
                if game_type_group != "Regular Season":
                    st.info(GAME_TYPE_GROUP_NOTE.format(game_type_group))
                ...
                download_button(
                    df, f"team_pitching_splits_{split_type}", f"team_pitching_splits_download_{idx}"
                )
